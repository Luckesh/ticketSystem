...
if ($result->num_rows > 0) {
  echo "<h2>Vaše hlásenia</h2>";
    ...
    while($row = mysqli_fetch_assoc($result)) {
      ...
      echo '</ul>';
      echo '</div>';
      echo '<div class="lowerBoxSolvNew">';
      ...
      echo '</div>';
      echo '<div class="lowerBoxSolve">';
      echo '<p class="tip">Vyriešiť:</p>';
      echo '<button class="solveBtn" onclick=displayModal('.$row["IDticket"].',0)>&#x2714</button>';
      echo '</div>';
      echo '<div class="lowerBoxIcon">';
      echo '<p class="tip">Kontakt:</p>';
      echo '<button class="mailBtn" onclick="displayModal('.$row["IDticket"].',1)"></button>';
      echo '</div>';
      echo '<div class="lowerBoxChange">';
      echo '<p class="tip">Presunúť:</p>';
      echo '<button class="changeBtn" onclick="displayModal('.$row["IDticket"].',2)"></button>';
      echo '</div>';
      echo '</div>';
    ...
    echo "<h2>Nemáte žiadne hlásenia na riešenie</h2>";
}
...
if ($result->num_rows > 0) {
  echo "<h2>Vyriešené hlásenia</h2>";
    ...
    while($row = mysqli_fetch_assoc($result)) {
      ...
      echo '<p>'.$row['riesitel'].'</p>';
      echo '</div>';
      echo '<div class="lowerBoxUcebna">';
      echo '<p class="tip">Ucebna:</p>';
      echo '<p>'.$row['ucebna_id'].'</p>';
      echo '</div>';
      echo '<div class="lowerBoxPc">';
      echo '<p class="tip">Pocitac:</p>';
      echo '<p>'.$row['pc_id'].'</p>';
      echo '</div>';
      echo '<div class="lowerBoxDue">';
